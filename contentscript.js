    ...
    pLines.forEach(line => {
        try {
            ...
            if (testdecoded) {
                ...
                line.innerHTML = `<a href=${fullydecoded}>${fullydecoded}</a> (decoded from base64) `
                //add revert button
    ...
    brLines.forEach(line => {
        try {
            ...
            if (testdecoded) {
                ...
                document.getElementById('symyabase64-'+id).innerHTML = `<a href=${fullydecoded}>${fullydecoded}</a> (decoded from base64) `
                // add revert button
                const revertButton = document.createElement("a")
                revertButton.href = "javascript:void(0)"
                revertButton.innerHTML = "Revert"
